...
local isWorldLadderOrPole = function(worldobjects)
    ...
    elseif type(worldobjects) == 'userdata' then
...
Ladders.doBuildMenu = function(player, context, worldobjects, test)
    ...
    local doneSquare = {}
    for _, obj in ipairs(worldobjects) do
        if obj:getSquare() and not doneSquare[obj:getSquare()] then
            doneSquare[obj:getSquare()] = true
            table.insert(squares, obj:getSquare())
        end
    end
    ...
    if climbOption then
        ...
        local is_down = false
        local is_ladder = isWorldLadderOrPole(worldobjects)
